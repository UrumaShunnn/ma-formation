...
function CheckItem({ text }: { text: string }) {
  return (
    <li style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
      <div style={{ width: 22, height: 22, borderRadius: "50%", background: "rgba(124,58,237,0.2)", border: "1px solid rgba(124,58,237,0.5)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1 }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", position: "relative", zIndex: 1 }}>
        ...
        >
          <div style={{ marginBottom: 28, textAlign: "center" }}>
            <span style={{ display: "inline-block", background: "linear-gradient(135deg, #7C3AED 0%, #5B21B6 100%)", borderRadius: 8, padding: "6px 20px", fontSize: 11, fontWeight: 700, color: "#FFFFFF", fontFamily: "Syne, sans-serif", letterSpacing: "0.15em", textTransform: "uppercase" }}>
              ✦ Accès à Vie ✦
            </span>
          </div>

          <h3 style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: 24, color: "#FFFFFF", textAlign: "center", marginBottom: 8, lineHeight: 1.25 }}>
            Formation Complète — Accès à Vie
          </h3>
          <p style={{ color: "#9CA3AF", textAlign: "center", fontSize: 15, marginBottom: 32, lineHeight: 1.6 }}>
          ...
          <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 14, marginBottom: 40 }}>
            {includes.map((item, i) => <CheckItem key={i} text={item} />)}
          </ul>
          ...
          <a
            ...
            style={{
              ...
              boxShadow: ctaHovered ? "0 0 48px rgba(124,58,237,0.6)" : "0 0 24px rgba(124,58,237,0.35)",
          ...
          <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 8, alignItems: "center" }}>
            <p style={{ color: "#6B7280", fontSize: 13, textAlign: "center" }}>🔒 Paiement sécurisé — Accès immédiat après confirmation</p>
            <p style={{ color: "#8B5CF6", fontSize: 13, fontWeight: 600, textAlign: "center", fontFamily: "Syne, sans-serif" }}>⚡ Places limitées — 12 membres actifs cette semaine</p>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 580, margin: "40px auto 0", width: "100%", display: "flex", flexDirection: "column", gap: 12 }}>
          {miniTestimonials.map((t, i) => (
            ...
            >
              <div style={{ width: 36, height: 36, borderRadius: "50%", background: "linear-gradient(135deg, #7C3AED, #5B21B6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: "#FFFFFF", fontFamily: "Syne, sans-serif", flexShrink: 0 }}>
                {t.name.split(" ")[0][0]}{t.name.split(" ")[1]?.[0] ?? ""}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                  <span style={{ fontFamily: "Syne, sans-serif", fontWeight: 600, fontSize: 13, color: "#FFFFFF" }}>{t.name}</span>
                  <span style={{ color: "#F59E0B", fontSize: 12, letterSpacing: 1 }}>★★★★★</span>
                </div>
                <p style={{ color: "#9CA3AF", fontSize: 13, lineHeight: 1.4, margin: 0 }}>&ldquo;{t.text}&rdquo;</p>
              </div>
            </div>
          ))}
        </div>
